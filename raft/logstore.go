...
//
// Indexes are 1-based and contiguous. After Compact(index, term, snap),
// FirstIndex() == index+1 and Term(index) still answers term, so a leader
// can build prevLogTerm for the first live entry.
type LogStore interface {
	...
	// Compact discards entries up to and including index, retaining
	// snapshot as the state at that point. index may lie beyond
	// LastIndex, in which case the whole log is replaced.
	Compact(index, term uint64, snapshot []byte) error
